...
function AnnouncementItem({ ann }: { ann: Announcement }) {
  ...
  return (
    <button style={s.item} onClick={() => navigate(`/announcements/${ann.id}`)}>
      ...
      <IoChevronForward size={16} color={marriageColors.gray400} />
    </button>
  );
}
...
const s: Record<string, CSSProperties> = {
  page: { display: 'flex', flexDirection: 'column', padding: '8px 16px', gap: 8 },
  item: {
    ...
    boxShadow: '0 1px 4px rgba(0,0,0,0.05)', cursor: 'pointer', textAlign: 'left',
  },
  iconBox: { width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  body: { flex: 1, display: 'flex', flexDirection: 'column', gap: 4, minWidth: 0 },
  titleRow: { display: 'flex', alignItems: 'center', gap: 6 },
  badge: { fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 6, flexShrink: 0, fontFamily: 'BMJUA, sans-serif' },
  title: {
    fontSize: 14, fontWeight: 600, color: marriageColors.gray800,
    fontFamily: 'BMJUA, sans-serif', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
  },
