    ...
    <div style={s.page}>
      <div style={s.header}>
        ...
        <div style={s.authorInfo}>
          <span style={s.authorName} onClick={() => navigate(`/profile/${encodeURIComponent(post.author.name)}`)}>
            {post.author.name}
          </span>
        ...
        <div style={s.moreWrap}>
          ...
          {menuOpen && (
            <>
              ...
              <div style={s.menu}>
                <button style={s.menuItem} onClick={() => { setMenuOpen(false); setShowReport(true); }}>
                  🚨 신고하기
                </button>
              </div>
            </>
      ...
      {post.image && <img src={post.image} alt="" style={s.image} />}

      <div style={s.actions}>
        <button style={s.actionBtn} onClick={handleLike} className={animating ? 'animate-heart-bounce' : ''}>
          <span style={{ color: liked ? '#ef4444' : marriageColors.gray400, fontSize: 22, transition: 'color 0.2s' }}>
            {liked ? '❤️' : '🤍'}
          </span>
          <span style={{ ...s.actionCount, color: liked ? '#ef4444' : marriageColors.gray500 }}>{likeCount}</span>
        </button>
...
const s: Record<string, CSSProperties> = {
  page: { padding: '16px' },
  notFound: { padding: 32, textAlign: 'center', color: marriageColors.gray400, fontFamily: 'BMJUA, sans-serif' },
  header: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 },
  avatar: { width: 44, height: 44, borderRadius: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  avatarText: { fontSize: 18, fontWeight: 700, color: marriageColors.gray700 },
  authorInfo: { display: 'flex', flexDirection: 'column', gap: 2, flex: 1 },
  authorName: { fontSize: 15, fontWeight: 700, color: marriageColors.gray800, fontFamily: 'BMJUA, sans-serif', cursor: 'pointer' },
  date: { fontSize: 12, color: marriageColors.gray400 },
  moreWrap: { position: 'relative' },
  moreBtn: { background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: marriageColors.gray400, padding: '4px 8px', letterSpacing: 1, fontFamily: 'inherit' },
  ...
  content: { fontSize: 15, lineHeight: 1.7, color: marriageColors.gray700, fontFamily: 'BMHANNAPro, sans-serif', marginBottom: 16 },
  image: { width: '100%', borderRadius: 10, objectFit: 'cover', display: 'block', marginBottom: 16 },
  actions: { display: 'flex', alignItems: 'center', gap: 4, paddingBlock: 10, borderTop: `1px solid ${marriageColors.gray100}`, borderBottom: `1px solid ${marriageColors.gray100}`, marginBottom: 20 },
  ...
  actionCount: { fontSize: 14, color: marriageColors.gray500 },
  commentSection: { padding: '16px 0' },
  commentPlaceholder: { textAlign: 'center', color: marriageColors.gray400, fontFamily: 'BMJUA, sans-serif', fontSize: 14 },
};
