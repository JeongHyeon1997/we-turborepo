...
import { confirmMarriage } from '../api/couple.api';
...
export function CoupleConfirmPage() {
  ...
  async function handleAccept() {
    ...
    setError('');
    try {
      const res = await confirmMarriage({ inviteCode: code });
...
const s: Record<string, CSSProperties> = {
  ...
  card: { width: '100%', maxWidth: 360, backgroundColor: marriageColors.white, borderRadius: 24, padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, boxShadow: `0 8px 32px ${ACCENT}2e`, border: `1px solid ${marriageColors.primary100}` },
  ...
  declineBtn: { flex: 1, padding: '13px 0', borderRadius: 14, border: `1px solid ${marriageColors.gray200}`, backgroundColor: marriageColors.white, fontSize: 15, fontFamily: 'BMJUA, sans-serif', color: marriageColors.gray500, cursor: 'pointer' },
  acceptBtn: { flex: 2, padding: '13px 0', borderRadius: 14, border: 'none', backgroundColor: ACCENT, fontSize: 15, fontWeight: 700, fontFamily: 'BMJUA, sans-serif', color: marriageColors.white, cursor: 'pointer', boxShadow: `0 4px 12px ${ACCENT}55` },
};
